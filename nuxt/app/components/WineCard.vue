...
import type { Wine } from '~~/shared/mock/wines'

defineProps<{ wine: Wine }>()
</script>
...
<template>
  <article class="relative flex min-w-0 flex-col items-center pb-5 text-center">
    ...
    <div class="relative z-[1] mt-3 text-xs uppercase tracking-[0.18em] text-bordeaux">{{ wine.meta.wineType }}</div>
    <h3 class="relative z-[1] mt-1 flex min-h-[2.3em] items-center justify-center font-serif text-[22px] leading-[1.08]">{{ wine.title }}</h3>
    <div class="relative z-[1] text-sm text-black/70">{{ wine.meta.variety }}</div>
    <div class="relative z-[1] mt-1 text-sm text-black/60">{{ wine.meta.method }}</div>
    <div class="relative z-[1] text-sm text-black/60">{{ wine.meta.year }} · {{ wine.meta.terroirName }}</div>
  </article>
</template>
