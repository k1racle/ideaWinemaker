...
const openSelect = (event: MouseEvent) => {
  ...
  const select = (event.currentTarget as HTMLLabelElement).querySelector('select')
  if (!select) return
  ...
  try {
    ...
    // The label's native behavior remains the fallback for unsupported contexts.
  }
}
</script>
  ...
  <form class="mt-7 text-left" @submit.prevent="emit('submit')">
    ...
    <div class="mx-auto mt-7 flex max-w-[920px] items-stretch overflow-visible rounded-[40px] border border-border-strong max-[780px]:grid max-[780px]:grid-cols-2 max-[780px]:rounded-3xl max-[520px]:grid-cols-1">
      <div class="grid min-w-0 flex-1 grid-cols-5 max-[780px]:contents">
        <label class="relative cursor-pointer rounded-l-[40px] px-2.5 py-2.5 text-center transition-colors duration-200 hover:bg-ink/[0.05] after:absolute after:right-0 after:top-[20%] after:h-[60%] after:w-px after:bg-border max-[780px]:rounded-none max-[780px]:rounded-tl-3xl max-[780px]:border-b max-[780px]:border-border max-[780px]:after:hidden max-[520px]:rounded-t-3xl" @click="openSelect">
        ...
        <label class="relative cursor-pointer px-2.5 py-2.5 text-center transition-colors duration-200 hover:bg-ink/[0.05] after:absolute after:right-0 after:top-[20%] after:h-[60%] after:w-px after:bg-border max-[780px]:border-b max-[780px]:border-border max-[780px]:after:hidden" @click="openSelect">
