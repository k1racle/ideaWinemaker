...
<template>
  <main class="pt-8">
    <div class="container-iw">
      <section><NuxtLink to="/vinodely" class="text-xs uppercase tracking-[0.16em] text-bordeaux">← Все виноделы</NuxtLink></section>
      <section class="mt-8 grid grid-cols-[minmax(0,380px)_minmax(0,1fr)] items-start gap-[clamp(28px,4vw,48px)] max-[900px]:grid-cols-1">
        ...
        <div class="min-w-0">
          ...
          <p class="mt-3 text-[12px] uppercase tracking-[0.2em] text-bordeaux">{{ winemaker.meta.location }}</p>
          <blockquote class="mt-8 max-w-[700px] border-l-2 border-bordeaux px-6 py-2 font-serif text-[clamp(19px,2vw,26px)] leading-[1.4] text-[#7e6e5a]">{{ winemaker.quote }}</blockquote>
          <h2 class="mt-5 font-sans text-[12px] font-normal uppercase leading-none tracking-[0.02em] text-bordeaux">{{ winemaker.meta.aboutBrand }}</h2>
          <div class="mt-8">
            <h2 class="font-serif text-[32px] uppercase tracking-[0.12em]">Биография</h2>
            <div class="mt-5 space-y-4 leading-relaxed text-black/85"><p v-for="paragraph in winemaker.biography" :key="paragraph">{{ paragraph }}</p></div>
          </div>
        </div>
      </section>
