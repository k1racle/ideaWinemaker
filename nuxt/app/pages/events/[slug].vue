    ...
    <div class="container-iw">
      ...
      <section class="mt-[52px] grid grid-cols-[minmax(280px,0.75fr)_minmax(0,1.25fr)] gap-8 max-[800px]:grid-cols-1">
        <aside class="order-1 space-y-5 max-[800px]:order-2">
          ...
          <div class="border-t border-border pt-4 text-sm leading-relaxed">
            <div class="text-[11px] uppercase tracking-[0.14em] text-primary">Дата и время</div>
            <p class="mt-1">{{ event.meta.dateStart }} – {{ event.meta.dateEnd }}<br>Время {{ event.meta.timeStart }}–{{ event.meta.timeEnd }}</p>
          </div>
          <div class="border-t border-border pt-4 text-sm leading-relaxed">
            <div class="text-[11px] uppercase tracking-[0.14em] text-primary">Место</div>
            <p class="mt-1">{{ event.meta.city }}<br>{{ event.meta.location }}, {{ event.meta.address }}</p>
          </div>
          <div class="border-t border-border pt-4 text-sm"><span v-if="event.meta.price">Цена: {{ event.meta.price.toLocaleString('ru-RU') }} {{ event.meta.currency }}</span><span v-else>Бесплатно</span> · {{ event.meta.ageLimit }}</div>
          <div class="border-t border-border pt-4 text-sm leading-relaxed">
            <div class="text-[11px] uppercase tracking-[0.14em] text-primary">Организатор</div>
            <p class="mt-1">{{ event.meta.organizer }}</p>
            <a :href="event.meta.organizerUrl" target="_blank" rel="noopener noreferrer" class="text-primary">{{ event.meta.organizerUrl.replace(/^https?:\/\//, '').replace(/\/$/, '') }}</a>
          </div>
        </aside>
