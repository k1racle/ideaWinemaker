...
import type { AdminContentOverview, AdminWineEditorData } from '~~/shared/types/content'
...
if (!Number.isInteger(recordId) || recordId <= 0) {
  throw createError({ statusCode: 400, statusMessage: 'Некорректный ID вина' })
}

const { data: initial, error: loadError } = await useFetch<AdminWineEditorData>(`/api/admin/wines/${recordId}`)
if (loadError.value) {
  throw createError({
    statusCode: loadError.value.statusCode || 500,
    statusMessage: loadError.value.statusMessage || 'Не удалось загрузить вино',
  })
}
if (!initial.value) throw createError({ statusCode: 404, statusMessage: 'Вино не найдено' })
...
const form = reactive({
  winemakerId: initial.value.winemakerId,
  terroirId: initial.value.terroirId,
  slug: initial.value.slug,
  title: initial.value.title,
  excerpt: initial.value.excerpt,
  content: initial.value.content,
  image: initial.value.image,
  authorQuote: initial.value.authorQuote,
  details: initial.value.details.map(group => ({ title: group.title, items: [...group.items] })),
...
const submit = async () => {
  ...
  errorMessage.value = ''
  try {
    const updated = await $fetch<{ id: number, slug: string }>(`/api/admin/wines/${recordId}`, {
      method: 'PATCH',
      body: form,
    })
    ...
    await navigateTo(isEffectivelyVisible ? `/wine/${updated.slug}` : '/admin')
  } catch (error) {
...
<template>
  <main class="container-iw py-10">
    ...
    <div class="mt-6 flex max-w-5xl flex-wrap items-end justify-between gap-5">
      <div>
        <h1 class="font-serif text-[clamp(36px,5vw,52px)]">Редактировать вино</h1>
        <p class="mt-2 text-sm text-ink/60">Можно изменить и винодела: обязательная связь обновится вместе с остальными полями.</p>
      </div>
    ...
    <form class="mt-8 max-w-5xl space-y-8" novalidate @submit.prevent="submit">
      ...
      <div class="flex flex-wrap gap-4">
        <button type="submit" :disabled="pending || !overview.winemakers.length || !overview.terroirs.length" class="min-h-12 cursor-pointer rounded-full bg-primary px-7 text-sm uppercase tracking-[0.14em] text-canvas disabled:cursor-not-allowed disabled:opacity-50">{{ pending ? 'Сохранение…' : 'Сохранить изменения' }}</button>
