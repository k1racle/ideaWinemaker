...
<template>
  <header class="sticky top-0 z-50 border-b border-border bg-canvas/95 backdrop-blur-md">
    <div class="container-iw grid grid-cols-[15%_70%_15%] items-center justify-between py-[18px] max-nav:grid-cols-[minmax(0,1fr)_auto] max-nav:py-[14px]">
      ...
      <nav class="flex flex-wrap items-center justify-center gap-[26px] text-[11px] uppercase tracking-[0.22em] max-nav:hidden">
        <NuxtLink
          ...
          :key="item.url"
          :to="item.url"
          class="opacity-[0.78] transition duration-200 hover:text-primary hover:opacity-100"
          :class="{ 'text-primary opacity-100': isActive(item.url) }"
        >{{ item.title }}</NuxtLink>
      </nav>
      ...
      <button
        ...
        class="relative hidden size-[42px] cursor-pointer justify-self-end rounded-[10px] border border-border-strong bg-canvas nav:hidden max-nav:block"
        @click="menuOpen = !menuOpen"
      >
        <span
          class="absolute left-1/2 h-[2px] w-5 -translate-x-1/2 bg-ink transition-all duration-300"
          :class="menuOpen ? 'top-1/2 -translate-y-1/2 rotate-45' : 'top-[13px]'"
        />
        <span
          class="absolute left-1/2 top-1/2 h-[2px] w-5 -translate-x-1/2 -translate-y-1/2 bg-ink transition-all duration-300"
  ...
  <Teleport to="body">
    <div
      id="mobileMenu"
      class="fixed inset-x-0 bottom-0 top-[71px] z-40 hidden bg-canvas transition-all duration-300 nav:hidden max-nav:block"
      ...
      >
        <nav class="grid text-left font-sans text-[16px] uppercase leading-none tracking-[0.14em]">
          <NuxtLink
            ...
            :key="item.url"
            :to="item.url"
            class="border-b border-border py-4 transition-colors duration-200"
            :class="isActive(item.url) ? 'text-primary' : 'text-ink'"
          >{{ item.title }}</NuxtLink>
        </nav>

        <div class="mt-auto pt-8">
          <p class="mb-4 text-center text-[10px] uppercase tracking-[0.18em] text-ink/50">Мы в социальных сетях</p>
          <div class="flex flex-wrap items-center justify-center gap-2.5">
            <a
              ...
              class="inline-flex size-10 items-center justify-center rounded-xl bg-surface-accent transition-colors hover:bg-surface"
            >
